...
public class SweepPattern : AttackPatternBase
{
    ...
    protected override IEnumerator Prepare()
    {
        ...
        if (player != null)
        {
            Vector2 to = player.position - transform.position; //Player를 향한 방향 값 저장
        ...
        if (telegraph != null)
        {
            telegraph.ShowSweep(transform.position, cachedDir, range, width); //플레이어 방향으로 패턴 예고 표시
        }
    ...
    private void OnDrawGizmosSelected() //디버깅용 함수
    {
