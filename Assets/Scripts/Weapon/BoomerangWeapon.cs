...
public class BoomerangWeapon : MonoBehaviour
{
    ...
    [SerializeField]
    private float attackCooldown = 1.0f; //부메랑 발사 쿨타임 (데미지 적용 쿨타임과는 별개)
    ...
    [SerializeField]
    private float aimRange = 8.0f; //적 탐지 범위

    [SerializeField]
    private float spreadDeg = 16.0f; //발사 방향의 퍼짐 각도 (부메랑 개수가 여러개일 때 적용)
    ...
    Vector2 FindAimDirection()
    {
        ...
        Transform bestTarget = null; //가장 가까운 적의 이지 정보를 저장할 변수
        ...
        for(int i = 0; i < hits.Length; ++i)
        {
            ...
            Vector2 targetPos = c.bounds.center; //감지된 적의 위치 정보를 가져옴. collider의 중심 위치값 획득
