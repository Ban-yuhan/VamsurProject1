...
public class ObjectPool : MonoBehaviour
{
    ...
    private readonly Queue<PooledObject> queue = new Queue<PooledObject>(); //readonly : queue 자료구조를 읽기 전용으로 설정함.
    private int totalCreated;


    /// <summary>
    /// PoolManager가 키로 이 풀을 등록할 때 사용하기 위한 함수
    /// </summary>
    ...
    private PooledObject CreateOneInactive()
    {
        ...
        ++totalCreated; //생성한 총알 수 증가
        obj.gameObject.SetActive(false); //총알을 생성한 후, 비활성화
        queue.Enqueue(obj); //생성한 총알을 queue 자료구조에 저장해놓음
        return obj;
    }
}
