...
/// 선별 후보에서 희귀도/가중치로 선별 → 최대 레벨인 항목 제외 → 중복을 포함해서 N개 뽑기(지금 당장은 3개)
/// </summary>
public class UpgradePool : MonoBehaviour
{
    ...
    [Header("Progress Scaling")] //시간이 지남에 따라 증가하는 가중치
    [SerializeField]
    ...
    public List<UpgradeDefinition> RollOptions(int count, UpgradeRuntimeState state, float elapsedSeconds)
    {
        ...
        int wRare = Mathf.Max(1, Mathf.RoundToInt(wr)); //RoundToInt() : 반올림하여 정수로 반환
